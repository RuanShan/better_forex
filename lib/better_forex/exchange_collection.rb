module BetterForex
  class ExchangeCollection
    ...
    def initialize( symbols, fields )
      ...
      self.exchange_map = {}
      self.exchange_redis_store = ExchangeRedisStore.new()
    end
    ...
    def push_message( data, time )
      if self.push_count==0
        data.each_with_index{|item, i|
          symbol = item[0]
          exchange_map[i] = ExchangeDescription.new( symbol, fields, time, exchange_redis_store)
          exchange_map[i].push_message( item, time )
        }
      else
        data.each{|item|
          if item.length >1
            ...
            exchange.push_message( item, time )
          end
        }
      end
      self.push_count += 1
